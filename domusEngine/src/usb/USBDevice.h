...
namespace zigbee {
    ...
    public:
        DomusEngineUSBDevice(boost::asio::io_service &io, std::shared_ptr<ZDevices> &zDevices, AttributeDataContainer &attributeDataContainer,
                             libusb_context *usbContext, int deviceClass, int vendorID, int productID);
    ...
    public:
        ...
        void registerForAttributeCmd(NwkAddr , const EndpointID , ClusterID ,
                                     ZigbeeAttributeCmdId ,
        ...
        virtual void sendReqBind(NwkAddr destAddr, uint8_t outClusterAddr[Z_EXTADDR_LEN], EndpointID outClusterEP, ClusterID clusterID,
                                 uint8_t inClusterAddr[Z_EXTADDR_LEN], EndpointID inClusterEp) override;
