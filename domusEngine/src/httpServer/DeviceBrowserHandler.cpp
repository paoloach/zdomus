...
namespace zigbee {
    namespace http {
        ...
        DeviceBrowserHandler::DeviceBrowserHandler(zigbee::SingletonObjects &singletons) :restGetActions(singletons.getFixedPathContainer()){
            ...
            restPostActions.addActions(RestPath {"/bind/src/{srcDevice}/endpoint/{srcEndpoint}/cluster/{cluster}/dst/{dstDevice}/endpoint/{dstEndpoint}"},
                                       ExecuteBind{singletons, true});

            restDeleteActions.addActions(RestPath {"/bind/src/{srcDevice}/endpoint/{srcEndpoint}/cluster/{cluster}/dst/{dstDevice}/endpoint/{dstEndpoint}"},
                                         ExecuteBind{singletons, false});
        }
        ...
        void DeviceBrowserHandler::handleRequest(Poco::Net::HTTPServerRequest &request, Poco::Net::HTTPServerResponse &response) {
            ...
            try {
                ...
                if (request.getMethod() == "GET") {
                    ...
                    restDeleteActions.execute(std::move(pathReceived), serverRequest, response);
                }
