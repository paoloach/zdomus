...
namespace zigbee {
    ...
    JSZDevices::JSZDevices(ZDevices *zDevices, JSZDevice * jsDevice) :
    ...
    void JSZDevices::jsGetDevice(const v8::FunctionCallbackInfo<v8::Value> &info) {
        if (info.Length() != 1) {
            ...
            stream << "Invoking ZDevice.getDevice with " << info.Length()
                   << " parameters but expected 1 (the extended address of a valid ZDevice)";
