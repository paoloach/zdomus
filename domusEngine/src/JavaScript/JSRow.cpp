...
v8::Local<v8::Object> JSRow::createInstance(v8::Isolate* isolate, std::shared_ptr<DBRow>  dbRow) {
	if (createdObject.count(dbRow)){
		Local<Object> object = Local<Object>::New(isolate, createdObject[dbRow]);
		return object;
	}
...
void zigbee::JSRow::constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
	...
	try {
		std::shared_ptr<DBRow> dbRow( new DBRow());

		JSRow * This = (JSRow *) (Local<External>::Cast(info.Data())->Value());

		info.GetReturnValue().Set(This->createInstance(isolate, dbRow));
