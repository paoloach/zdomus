...
namespace zigbee {
    ...
    JSZDevice::JSZDevice(ZDevices_P zDevices, JSZEndpoint * jsEndpoint) :
    ...
    void JSZDevice::initJsObjectsTemplate(v8::Isolate *isolate, Handle<Object> &global) {
        ...
        Local<FunctionTemplate> zDeviceFunctionTemplate = FunctionTemplate::New(isolate, constructor,
    ...
    Local<Object> JSZDevice::createInstance(v8::Isolate *isolate, const ExtAddress &extAddress) {
        if (cache.count(extAddress) == 0) {
            ...
            Local<ObjectTemplate> zDeviceTempl = Local<FunctionTemplate>::New(isolate,
            ...
            zDeviceInstance->SetInternalField(2, External::New(isolate, jsEndpoint));
    ...
    void JSZDevice::jsGetEndpoints(const v8::FunctionCallbackInfo<v8::Value> &info) {
        ...
        for (const auto &endpoint : endpoints) {
            v8::Local<Object> zEndpoint = jsEndpoint->createInstance(info.GetIsolate(), *extAddress,
                                                                     endpoint.second.getEndpoint());
