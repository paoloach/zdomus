...
namespace zigbee {
    ...
    ExtAddress::ExtAddress(std::initializer_list<uint8_t> extAddr) {
        std::copy(std::begin(extAddr), std::end(extAddr), data.begin());
    }
    ...
    std::string ExtAddress::asString() const {
        ...
        stream << std::setw(2) << std::setfill('0') << std::hex << (int)data[Z_EXTADDR_LEN - 1];
