    ...
    namespace http {
        namespace test {
            ...
            public:
                ...
                MOCK_METHOD2(sendFile, void(
                        const std::string &path,
                        const std::string &mediaType));
                MOCK_METHOD2(sendBuffer, void(
                        const void *pBuffer, std::size_t
                        length));
                MOCK_METHOD2(redirect, void(
                        const std::string &uri, HTTPStatus
                        status ));
                MOCK_METHOD1(redirect, void(
                        const std::string &uri));
                MOCK_METHOD1(requireAuthentication, void(
