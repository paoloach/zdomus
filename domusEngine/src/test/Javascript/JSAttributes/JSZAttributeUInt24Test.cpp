...
void JSZAttributeUInt24Test::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttributeUint24>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zclUInt24AttributeMock, getZCLType()).Times(AnyNumber()).WillOnce(Return(ZCLTypeDataType::ZCLTypeUInt24));
}
