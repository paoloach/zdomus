...
void JSZAttributeBooleanTest::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttributeBool>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zcl_boolean_AttributeMock, getZCLType()).Times(AnyNumber()).WillOnce(Return(ZCLTypeDataType::ZCLTypeBool));
}
