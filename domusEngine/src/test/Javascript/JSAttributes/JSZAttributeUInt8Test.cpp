...
void JSZAttributeUInt8Test::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttributeUint8>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zclUint8AttributeMock, getZCLType()).Times(AnyNumber()).WillOnce(Return(ZCLTypeDataType::ZCLTypeUInt8));
}

void JSZAttributeUInt8Test::TearDown() {
	jsZAttribute.reset();
	zclUint8AttributeMock.reset();
	JSTest::TearDown();
}
...
TEST_F( JSZAttributeUInt8Test, setValue) {
	...
	setInitExpectation(zDevice, zclUint8AttributeMock);
	EXPECT_CALL(*zclUint8AttributeMock, sendValue(expectedValue));
