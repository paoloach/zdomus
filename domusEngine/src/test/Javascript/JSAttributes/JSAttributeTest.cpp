...
std::map<ZCLAttribute::Status, std::string> expectedStatusMap { { ZCLAttribute::Available, "available" }, { ZCLAttribute::NotAvailable, "notAvailable" }, {
...
void JSAttributeTest::createIstanceTest(const std::string& attributeName, std::shared_ptr<JSZAttribute>& jsZAttribute, std::shared_ptr<ZCLAttribute> attributeMock) {
...
void JSAttributeTest::setInitExpectation(const ZDevice & zDevice, const std::shared_ptr<ZCLAttribute> & attributeMock) {
	EXPECT_CALL(*zDevices, exists(extAddress)).WillOnce(Return(true));
	EXPECT_CALL(*zDevices, getDevice(extAddress)).WillOnce(ReturnRef(zDevice));
	EXPECT_CALL(*clusterTypeFactoryMock, getCluster(CLUSTER_ID,zigbeeDevice, ENDPOINT_ID, NWK_ADDRESS)).WillOnce(Return(cluster));
	EXPECT_CALL(*cluster, getAttribute(ATTRIBUTE0_ID)).WillOnce(Return(attributeMock));
...
void JSAttributeTest::getStatusTest(ZCLAttribute::Status statusExpected, std::shared_ptr<JSZAttribute>& jsZAttribute, std::shared_ptr<ZCLAttribute> attributeMock) {
	ZDevice zDevice { createZDevice() };
	std::stringstream stream { };
...
void JSAttributeTest::requestValueWithCallbackTest(std::shared_ptr<JSZAttribute>& jsZAttribute, std::shared_ptr<ZCLAttribute> attributeMock, const OnChangeSignal & changeSignal) {
	ZDevice zDevice { createZDevice() };
	std::stringstream stream;
...
void JSAttributeTest::requestValueWithCallbackTestOnlyFirstTime(std::shared_ptr<JSZAttribute>& jsZAttribute, std::shared_ptr<ZCLAttribute> attributeMock,
		const OnChangeSignal& changeSignal) {
	ZDevice zDevice { createZDevice() };
	std::stringstream stream;
	stream << "var f = function (){\n";
	stream << "    var log = Log();\n";
