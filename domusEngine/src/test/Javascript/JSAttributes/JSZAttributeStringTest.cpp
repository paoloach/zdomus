...
void JSZAttributeStringTest::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttributeString>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zcl_string_AttributeMock, getZCLType()).Times(AnyNumber()).WillOnce(Return(ZCLTypeDataType::ZCLTypeStringChar));
}
