...
void JSZAttributeInt16Test::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttributeInt16>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zcl_int16_AttributeMock, getZCLType()).Times(AnyNumber()).WillOnce(Return(ZCLTypeDataType::ZCLTypeSInt16));
}
