...
void JSZAttribute8BitBitmapTest::SetUp() {
	JSTest::SetUp();
	jsZAttribute = std::make_shared<JSZAttribute8BitBitmap>(zDevices, zigbeeDevice, clusterTypeFactoryMock);
	...
	EXPECT_CALL(*zcl_bitmap8bit_AttributeMock, getZCLType()).Times(AnyNumber()).WillRepeatedly(Return(ZCLTypeDataType::ZCLType8bitBitmap));
}
