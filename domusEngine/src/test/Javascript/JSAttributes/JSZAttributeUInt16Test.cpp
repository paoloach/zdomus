...
using ActionFunction = ::testing::Action<boost::signals2::connection(boost::signals2::slot<void(), function<void()> >)> ;
...
TEST_F( JSZAttributeUInt16Test, getValue) {
	...
	EXPECT_CALL(*zclUint16Attribute, getValue()).WillOnce(Return(boost::any(expectedValue)));
...
TEST_F( JSZAttributeUInt16Test, setValue) {
	...
	setInitExpectation(zDevice, zclUint16Attribute);
	EXPECT_CALL(*zclUint16Attribute, sendValue(expectedValue));
