...
namespace zigbee {
    namespace test {
        ...
        static std::string createStringPropertiesUpdateCmdRequest(const RequestAttribute &requestAttribute1,
                                                                  const RequestAttribute &requestAttribute2) {
            ptree comandProperty{};
            stringstream outStream{};
        ...
        TEST_F(ComandFactoryTest, parserError) {
            ...
            std::vector<Comand_P> comands = ComandFactory::createComands("NO JSON", zDevices, zigbeeDevice,
                                                                         attributeData);
        ...
        TEST_F(ComandFactoryTest, DeviceUpdateCmd) {
            ...
            auto  cmd = comands[0].get();
            ASSERT_THAT(typeid(*cmd).name(), StrEq(typeid(CmdGetUpdate).name()));
        }
        ...
        TEST_F(ComandFactoryTest, PropertiesUpdateCmd) {
            ...
            ASSERT_THAT(comands.size(), 1);
            auto  cmd = comands[0].get();
            ASSERT_THAT(typeid(*cmd).name(), StrEq(typeid(CmdUpdateAttribute).name()));
        }
        ...
        TEST_F(ComandFactoryTest, RequestAttributeCmd) {
            ...
            ASSERT_THAT(comands.size(), 1);
            auto  cmd = comands[0].get();
            ASSERT_THAT(typeid(*cmd).name(), StrEq(typeid(CmdRequestAttribute).name()));
        }
        ...
        TEST_F(ComandFactoryTest, MultipleRequestAttributeCmd) {
            ...
            auto  cmd0 = comands[0].get();
            auto  cmd1 = comands[1].get();
            ASSERT_THAT(typeid(*cmd0).name(), StrEq(typeid(CmdRequestAttribute).name()));
            ASSERT_THAT(typeid(*cmd1).name(), StrEq(typeid(CmdRequestAttribute).name()));
        }
        ...
        TEST_F(ComandFactoryTest, RequestSendCmd) {
            ...
            ASSERT_THAT(comands.size(), 1);
            auto  cmd0 = comands[0].get();
            ASSERT_THAT(typeid(*cmd0).name(), StrEq(typeid(CmdSendCmd).name()));
        }
