...
namespace zigbee {
    ...
    SerialDriver2::SerialDriver2(const std::string &port, SingletonObjects *singletonObjects, std::chrono::seconds timeout) : ZigbeeDevice(timeout),
                                                                                                                              singletonObjects(singletonObjects), port(port) {
    ...
    // n bytes -> data values
    void
    SerialDriver2::writeAttribute(NwkAddr nwkAddrs, const EndpointID endpoint, ClusterID cluster, ZigbeeAttributeId attributeId, ZCLTypeDataType dataType, uint8_t dataValueLen,
                                  uint8_t *dataValue) {
        if (serialFd >= 0) {
            PacketSend data;
    ...
    void SerialDriver2::requestActiveEndpoints(NwkAddr nwkAddr) {
        BOOST_LOG_TRIVIAL(info) << "Request active endpoints for device " << nwkAddr;
        if (serialFd >= 0) {
            PacketSend data;
