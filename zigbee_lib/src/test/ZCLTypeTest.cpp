...
namespace zigbee {
    ...
    TEST_F(ZCLTypeTest, ZigbeeAttributeIdsRead) {
        std::stringstream stream;
        stream << "3A,4B,20string";
        ZigbeeAttributeIds zigbeeAttributeIds;
        std::string other;

        stream >> zigbeeAttributeIds >> other;


        ASSERT_THAT(zigbeeAttributeIds.size(), Eq(3));
        ASSERT_THAT(zigbeeAttributeIds[0], Eq(0x3A));
        ASSERT_THAT(zigbeeAttributeIds[1], Eq(0x4B));
        ASSERT_THAT(zigbeeAttributeIds[2], Eq(0x20));
        ASSERT_THAT(other, Eq("string"));
    }
