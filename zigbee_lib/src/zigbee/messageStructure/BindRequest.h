...
struct __attribute__ ((__packed__)) BindRequest {
	BindRequest(NwkAddr destAddr, uint8_t outClusterAddr[Z_EXTADDR_LEN],EndpointID outClusterEP,ClusterID clusterID, uint8_t inClusterAddr[Z_EXTADDR_LEN],EndpointID inClusterEp) noexcept :generticDataMsg(REQ_ADD_BIND_TABLE_ENTRY),
			destAddr(destAddr.getId()),outClusterEP(outClusterEP.getId()),clusterID(clusterID.getId()), inClusterEp(inClusterEp.getId()) {
