...
namespace zigbee {
    ...
    public:
        ...
        void sendReqBind(NwkAddr destAddr, uint8_t outClusterAddr[Z_EXTADDR_LEN], EndpointID outClusterEP, ClusterID clusterID, uint8_t inClusterAddr[Z_EXTADDR_LEN],
    ...
    inline void zigbee::USBDevice::sendData(const T &data) {
        ...
        if (handle != nullptr) {
            ...
            int result = libusb_bulk_transfer((libusb_device_handle *) handle, BULK_ENDPOINT_OUT,
                                              (unsigned char *) &data, sizeof(data), &transfered, 10);
            if (result == 0) {
