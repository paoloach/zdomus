...
namespace zigbee {
    ...
    ZCL_int16_Attribute::ZCL_int16_Attribute(ZigbeeDevice * zigbeeDevice, Cluster *parent,
    ...
    std::any ZCL_int16_Attribute::getValue() const {
        if (status != Available) {
    ...
    void ZCL_int16_Attribute::internalSetValue(std::shared_ptr<AttributeStatusRecord> rawData) {
        value = boost::endian::little_to_native( *(int16_t *) rawData->data.begin());
    }
