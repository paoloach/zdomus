...
namespace zigbee {
    ...
    MainWindow::MainWindow(libusb_context *usbContext) :
        ...
        zigbeeDevice->registerForAnnunceMessage([this](AnnunceMessage *annunceMsg){this->annunceDevice(annunceMsg);});
        zigbeeDevice->registerForSimpleDescMessage([this](SimpleDescMessage* simpleDesc){this->addToGlobalList(simpleDesc);});
        zigbeeDevice->registerForBindTableMessage([this](std::shared_ptr< BindTableResponseMessage> bindTable){this->bindTableMsg(bindTable);});

        ui.setupUi(this);
        timer = new QTimer();
        timerConnection = new QTimer();

        connect(timerConnection, SIGNAL(timeout()), SLOT(checkUSBConnect()));
    ...
    void MainWindow::addToGlobalList(SimpleDescMessage *simpleDesc) {
        ...
        std::cout << "Simple desc: endpoint=0x" << (int) simpleDesc->endpoint << "  in clusters: {";
        int clusterIndex=0;
        for (; clusterIndex < simpleDesc->numInClusters; clusterIndex++){
            std::cout << std::hex << simpleDesc->clustersList[clusterIndex] << " ";
        }
        std::cout << "}, out clusters {";
        for (; clusterIndex < simpleDesc->numOutClusters; clusterIndex++){
            std::cout << std::hex << simpleDesc->clustersList[clusterIndex] << " ";
        }
        std::cout << "}" << std::endl;
    ...
    void MainWindow::addToLightdeviceList(SimpleDescMessage *simpleDesc) {
        ...
        if (mapLightDeviceAction.count(deviceKey) == 0) {
            ...
            for (int i = 0; i < simpleDesc->numInClusters; i++) {
                ...
                std::shared_ptr<Cluster> cluster = ClusterTypeFactory::createCluster(ClusterID{*clusterIds}, zigbeeDevice, EndpointID{simpleDesc->endpoint},
...
//	dstAddr.addr.shortAddr = clusterIn->getNetworkAddress().getId();
    ...
    void MainWindow::checkUSBConnect() {
